...
export default async function RolesPage() {
  ...
  for (const row of memberCounts ?? []) {
    if (row.role_id) memberCountMap[row.role_id] = (memberCountMap[row.role_id] ?? 0) + 1
  }
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Define generic role descriptions. Assign them to team members to build a shared picture of team capability.
          </p>
        </div>
      ...
      {!roles || roles.length === 0 ? (
        <div className="rounded-lg border border-dashed p-12 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Create a role (e.g. "Mid Frontend Developer") and define its areas of responsibility.
          </p>
        </div>
      ) : (
