...
import { ChevronLeft, ChevronDown, X, Plus, ExternalLink, Trash2 } from 'lucide-react'
...
export function StrategyEditorClient({ initiative }: { initiative: StrategicInitiative }) {
  ...
  const saveField = useCallback(async (fields: Partial<StrategicInitiative>) => {
  ...
  const handleDelete = useCallback(async () => {
    setDeleting(true)
    const res = await fetch(`/api/strategies/${initiative.id}`, { method: 'DELETE' })
    if (!res.ok) {
  ...
  const handleDomainBlur = useCallback(async (e: React.FocusEvent<HTMLInputElement>) => {
    const val = e.target.value.trim()
    setDomain(val)
    await saveField({ domain: val || null })
  }, [saveField])

  const handleHorizonChange = useCallback(async (val: string) => {
    setHorizon(val)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="rounded-lg border bg-card overflow-hidden">
          ...
          <div className="px-5 pt-4 pb-3 space-y-2">
            <input
              className="w-full text-lg font-bold bg-transparent border-none outline-none placeholder:text-muted-foreground resize-none"
              value={title}
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button onClick={cycleStatus} title="Click to cycle status">
                <Badge variant={STATUS_COLORS[status] as 'default' | 'secondary' | 'outline' | 'destructive'}>
                  {status}
                </Badge>
              </button>
          ...
          <CollapsibleSection title="Tags" defaultOpen={true}>
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-muted-foreground">Enter or comma to add multiple</p>
            </div>
          </CollapsibleSection>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {HORIZON_OPTIONS.map((opt) => (
                    <button
                      key={opt}
                      type="button"
                      onClick={() => handleHorizonChange(horizon === opt ? '' : opt)}
