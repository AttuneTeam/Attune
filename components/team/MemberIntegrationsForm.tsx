...
const SUPPORTED_PROVIDERS: ProviderConfig[] = [
  ...
  },
  {
    ...
    hint: "Work email used in Cursor",
  },
  {
    key: "slack",
    label: "Slack",
    placeholder: "e.g. U012AB3CD",
    hint: "Slack member ID",
  },
  {
    key: "confluence",
    label: "Confluence",
    placeholder: "e.g. 557058:abc123",
    hint: "Atlassian account ID",
  },
  {
    key: "trello",
    label: "Trello",
    placeholder: "e.g. torvalds",
    hint: "Trello username",
  },
];
...
export function MemberIntegrationsForm({
  memberId,
  managerId,
  integrations,
  open: controlledOpen,
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-w-sm flex flex-col max-h-[90vh]">
        ...
        <form
          onSubmit={handleSave}
          className="overflow-y-auto flex-1 min-h-0 space-y-5 mt-1 pr-0.5"
        >
          {SUPPORTED_PROVIDERS.map(
            ({ key, label, placeholder, hint, configFields }) => (
              <div key={key} className="space-y-2">
                ...
                {configFields?.map((field) => (
                  <div
                    key={field.key}
                    className="space-y-1.5 pl-3 border-l ml-1"
                  >
                    <Label
                      htmlFor={`integration-${key}-${field.key}`}
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setConfigField(key, field.key, e.target.value)
                      }
